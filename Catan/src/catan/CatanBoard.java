...
public class CatanBoard extends JPanel{
	...
	protected void paintComponent(Graphics g) {
		for(Point point : allPoints){
			point.setPanelHeight(this.getHeight());
		}
		...
		for(int i = 0; i < 3; i++) {//first row 
			...
			if(firstPaint) {
				fields.get(i).setCoordinates((int)(upperLeftX + Math.sqrt(3) * side / 2), upperLeftY + side, this.getHeight()) ;
